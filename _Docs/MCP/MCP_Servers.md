...
**Note**: When a crew class is decorated with `@CrewBase`, the adapter lifecycle is managed for you:
...
⚠️ **Always ensure that you trust an MCP Server before using it.**
...
CrewAI supports the following transport mechanisms for MCP Servers:

1. **Stdio**: For local servers (communication via standard input/output between processes on the same machine)
2. **Server-Sent Events (SSE)**: For remote servers (unidirectional, real-time data streaming from server to client over HTTP)
3. **Streamable HTTPS**: For remote servers (flexible, potentially bi-directional communication over HTTPS, often utilizing SSE for server-to-client streams)

---

## Explore MCP Integrations

- [Simple DSL Integration (Recommended)](https://docs.crewai.com/en/mcp/dsl-integration): Use the simple `mcps=[]` field syntax for effortless MCP integration.
- [Stdio Transport](https://docs.crewai.com/en/mcp/stdio): Connect to local MCP servers via standard input/output. Ideal for scripts and local executables.
- [SSE Transport](https://docs.crewai.com/en/mcp/sse): Integrate with remote MCP servers using Server-Sent Events for real-time data streaming.
- [Streamable HTTP Transport](https://docs.crewai.com/en/mcp/streamable-http): Utilize flexible Streamable HTTP for robust communication with remote MCP servers.
- [Connecting to Multiple Servers](https://docs.crewai.com/en/mcp/multiple-servers): Aggregate tools from several MCP servers simultaneously using a single adapter.
- [Security Considerations](https://docs.crewai.com/en/mcp/security): Review important security best practices for MCP integration to keep your agents safe.

---

## Demo Repository

Checkout this repository for full demos and examples of MCP integration with CrewAI: [GitHub Repository CrewAI MCP Demo](https://github.com/tonykipkemboi/crewai-mcp-demo)
